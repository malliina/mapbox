...
class StyleBuilderTests extends BaseSuite with ClientFixture:
  ...
  http.test("update recipe".ignore) { client =>
    val r = makeRecipeFromNames(
      Seq("vaylaalueet", "vaylat", "turvalaitteet", "rajoitusalue_a", "taululinja"),
      client
    )
  ...
  def out[T: Encoder](json: T, to: Path = Paths.get("out.json")): Unit =
    val str = json.asJson.spaces2
